...
export default function RestaurantDetailPage() {
  ...
  const isFavorited = favorites.some((f) => f.restaurant_id === id);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // reviews failing shouldn't hide the restaurant
      }
      setLoading(false);
    };
    fetchData();
  ...
  const _idx = restaurant.name ? restaurant.name.charCodeAt(0) % PLACEHOLDER_IMAGES.length : 0;
  const imgSrc = restaurant.photos?.[0] || PLACEHOLDER_IMAGES[_idx];
        ...
        <div className="detail-info">
          ...
          <div className="detail-main">
            ...
            <div>
              ...
              {reviewActionError && (
                <div className="alert alert-error" style={{ marginBottom: 'var(--sp-sm)' }}>{reviewActionError}</div>
              )}
              ...
              {reviews.length === 0 ? (
                ...
                reviews.map((rev) => (
                  <ReviewCard
                    key={rev.id}
                    review={rev}
                    canDelete={isAuthenticated && user?.id === rev.user_id}
